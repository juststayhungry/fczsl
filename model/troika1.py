...
class Troika1(nn.Module):
    ...
    def __init__(self, config, attributes, classes, offset):
        ...
        width = self.clip.vision_width
        scale = width ** -0.5
        # self.attr_embedding = nn.Parameter(scale * torch.randn(width))
        # self.obj_embedding = nn.Parameter(scale * torch.randn(width))
        num_tokens =2
        self.num_tokens =num_tokens#attr and obj
        prompt_dim = width
        patch_size =  self.clip.vision_patch_size#.ConfigDict({'size': (self.clip.vision_patch_size, self.clip.vision_patch_size)})  
        # random initiate prompt:
        # if self.prompt_config.INITIATION == "random":
        val = math.sqrt(6. / float(3 * patch_size*patch_size + prompt_dim))  # noqa
            # self.prompt_embeddings = nn.Parameter(torch.zeros(
            #     1, num_tokens, prompt_dim))
            # # xavier_uniform initialization
            # nn.init.uniform_(self.prompt_embeddings.data, -val, val)
        total_d_layer = self.clip.visual.transformer.layers #config.transformer["num_layers"]-1
    ...
    def encode_image_with_adapter(self, x: torch.Tensor):
        ...
        x_pair = self.clip.visual.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device)
        # x_attr = self.attr_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device)
        # x_obj = self.obj_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device)
        x = torch.cat([x_pair,x], dim=1)  # shape = [*, grid ** 2 + 1, width]

        x = x + self.clip.visual.positional_embedding.to(x.dtype)
        # x = torch.cat([x_attr,x_obj,x], dim=1)
        x = self.clip.visual.ln_pre(x)

        # img_feature = self.clip.visual.transformer(x)
        for i_block in range(self.clip.visual.transformer.layers):
            B =x.shape[0]
            deep_prompt_emb = self.deep_prompt_embeddings[i_block].expand(B, -1, -1)
            if i_block ==0:
                x = torch.cat((
                x[:, :1, :],
                deep_prompt_emb,
            ...
            # adapt_x = self.additional_visual_params[i_block](x, add_residual=False)#在多头注意力中插adapter
            # residual = x
            ...
            # adapt_x = self.additional_visual_params[i_adapter](x, add_residual=False)#insert in Tr's FFN
            # residual = x
            # x = self.clip.visual.transformer.resblocks[i_block].mlp(
            #     self.clip.visual.transformer.resblocks[i_block].ln_2(x)
            # )
            # #在Tr的每一个block，即每一个transformer层的MHA与FFN都加了additional_visual_params
            # x = x + adapt_x + residual
            # self.deep_prompt_embeddings

        img_feature = self.clip.visual.ln_post(x)#
        if self.clip.visual.proj is not None:
            img_feature = img_feature @ self.clip.visual.proj

        return img_feature[:, 0, :],img_feature[:, 1, :],img_feature[:, 2, :], img_feature#attr,obj,pair,patch 
    ...
    def forward_for_open(self, batch, text_feats):
        ...
        b = batch_img.shape[0]
        # l, _ = idx.shape
        batch_pair,batch_obj,batch_attr, batch_patch = self.encode_image(batch_img.type(self.clip.dtype))
        batch_img_features = [batch_pair, batch_attr, batch_obj]
        batch_img_features = [batch_pair, self.attr_disentangler(batch_attr), self.obj_disentangler(batch_obj)]
    ...
    def forward(self, batch, idx,V_img):
        batch_img = batch[0].cuda()
        b = batch_img.shape[0]
        l, _ = idx.shape
        batch_attr,batch_obj,batch_pair, batch_patch = self.encode_image(batch_img.type(self.clip.dtype))
        # batch_img_features = [batch_pair, batch_attr, batch_obj]
        batch_img_features = [batch_pair, self.attr_disentangler(batch_attr), self.obj_disentangler(batch_obj)]
        normalized_img_features = [feats / feats.norm(dim=-1, keepdim=True) for feats in batch_img_features]
...
#             img_feature = img_feature @ self.clip.visual.proj
#         # breakpoint()
#         img_pooled = F.max_pool1d(img_feature[:, 1:, :].permute(0, 2, 1), kernel_size=256).squeeze(-1)
#         return img_feature[:, 0, :],img_pooled, img_feature
...
#         b = batch_img.shape[0]
#         # l, _ = idx.shape
#         batch_img,pool_img, batch_patch = self.encode_image(batch_img.type(self.clip.dtype))
#         batch_img_features = [batch_img, self.attr_disentangler(pool_img), self.obj_disentangler(batch_img)]
#         normalized_img_features = [feats / feats.norm(dim=-1, keepdim=True) for feats in batch_img_features]
