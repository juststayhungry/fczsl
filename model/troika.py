...
class Disentangler1(nn.Module):
    def __init__(self, emb_dim1,emb_dim2):
        ...
        self.fc1 = nn.Linear(emb_dim1,emb_dim2)
        self.bn1_fc = nn.BatchNorm1d(emb_dim2)
...
class Troika(nn.Module):
    ...
    def __init__(self, config, attributes, classes, offset):
        ...
        self.obj_disentangler = Disentangler(output_dim1)
        self.text_mlp = Disentangler1(output_dim,output_dim1)

        self.cmt = nn.ModuleList([CrossAttentionLayer(output_dim1, output_dim1//64, self.cross_attn_dropout) for _ in range(config.cmt_layers)])
        self.lamda = nn.Parameter(torch.ones(output_dim1) * config.init_lamda)
        self.patch_norm = nn.LayerNorm(output_dim1)
    ...
    def encode_text_for_open(self, idx):
        ...
        for i_element in range(self.token_ids.shape[0]):
            ...
            _text_features = self.text_mlp(_text_features)
    ...
    def forward_for_open(self, batch, text_feats,V_img):
    ...
    def forward(self, batch, idx,V_img):
        ...
        if V_img is not None:
            batch_img, batch_patch = pca_transform(batch_img, V_img),pca_transform(batch_patch, V_img)
        batch_img_features = [batch_img, self.attr_disentangler(batch_img), self.obj_disentangler(batch_img)]
        normalized_img_features = [feats / feats.norm(dim=-1, keepdim=True) for feats in batch_img_features]
